...
class Anchor3DHead(BaseModule, AnchorTrainMixin):
    ...
    def forward(self, feats):
        ...
        """
        if type(feats)==list or type(feats)==tuple:
            return multi_apply(self.forward_single, feats)
            
        elif type(feats)==torch.Tensor:
            cls_score, bbox_pred, dir_cls_preds = self.forward_single(feats)
            cls_score = [cls_score]
            bbox_pred = [bbox_pred]
            dir_cls_preds = [dir_cls_preds]
            return cls_score,bbox_pred,dir_cls_preds
        else:
            raise TypeError("The type must list or torch.Tensor")
