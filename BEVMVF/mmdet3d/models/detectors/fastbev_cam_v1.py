...
class FastBEV_CAM_V1(BaseDetector):
    def __init__(
        self,
        backbone=None,
        neck=None,
        ...
        if backbone:
            ...
            self.use_cam = True
            self.bbox_head_inchannel = bbox_head['feat_channels']
            self.re_cam_dim_cov = nn.Conv2d(neck_3d['out_channels'],self.bbox_head_inchannel,3,1,1)
        else:
            self.use_cam = False
        if neck:
        ...
        #cov layer
        if self.use_cam==True and self.use_pts==True:
            self.fusion_stream = ConvModule(
                pts_backbone['in_channels'],
                pts_backbone['in_channels'],
                7,
                stride=1,
                padding=3,
            ...
            self.cov2 =ConvModule(2*pts_backbone['in_channels'],
                                  pts_backbone['in_channels'],
                                  3,
                                  1,
                                  1,
        ...
        if self.freeze_img:
            ...
            if self.neck:
                for param in self.neck.parameters():
                    param.requires_grad = False
            if self.neck_3d:
                for param in self.neck_3d.parameters():
                    param.requires_grad = False
    ...
    def extract_feat(self, img, img_metas, points):
        ...
        if self.use_cam:
            img_bev_feat = self.extract_img_feat(img, img_metas)
            if self.use_pts:
                voxels, num_points, coors = self.voxelize(points)
                voxel_features = self.pts_voxel_encoder(voxels, num_points, coors)
                batch_size = coors[-1, 0] + 1
                point_feats = self.pts_middle_encoder(voxel_features, coors, batch_size)

                if img_bev_feat.shape[2:] != point_feats.shape[2:]:
                    self.img_bev_feat = F.interpolate(img_bev_feat, point_feats.shape[2:], mode='bilinear', align_corners=True)
                else:
                    self.img_bev_feat = img_bev_feat
                #将imgbev与pointfeat相加
                fusion_feats = torch.add(self.img_bev_feat,point_feats)
                #特征图进行线性变换
                fusion_feats = self.fusion_stream(fusion_feats)
                #共用卷积权重将imgbev进行二次特征提取
                img_bev_feat_2 = self.fusion_stream(self.img_bev_feat)
                #concate 
                fusion_feats = torch.cat([fusion_feats,img_bev_feat_2],dim=1)
                fusion_feats = self.cov2(fusion_feats)
                return fusion_feats

            else:
                if img_bev_feat.shape[2:] != [400,400]:
                    x = F.interpolate(img_bev_feat, [400,400], mode='bilinear', align_corners=True)
                x = self.pts_backbone(img_bev_feat)
                #if img_bev_feat.shape[1]!= self.bbox_head_inchannel:
                    #img_bev_feat = self.re_cam_dim_cov(img_bev_feat)
                x = self.pts_neck(x)
                return x
