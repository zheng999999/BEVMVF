...
model = dict(
    ...
    backbone=dict(
        ...
        init_cfg=dict(
            type='Pretrained',
            checkpoint=
            'test_api_inference/yolov3_mobilenetv2_mstrain-416_300e_coco_20210718_010823-f68a07b3.pth'
        )),
    neck=dict(
        type='YOLOV3Neck',
        num_scales=3,
    ...
    neck_fuse=dict(in_channels=288, out_channels=64),
    change_channel=16,
    neck_3d=dict(
...
db_sampler = dict(
    ...
    points_loader=dict(
        ...
        file_client_args=dict(backend='disk')))
img_norm_cfg = dict(
    mean=[93.8329, 98.7603, 95.8774],
    std=[78.7817, 80.1303, 81.1997],
    to_rgb=True)
train_pipeline = [
    dict(
        type='LoadPointsFromFile',
    ...
    dict(
        ...
        class_names=['Pedestrian', 'Cyclist', 'Car']),
    dict(type='KittiSetOrigin', point_cloud_range=[0, -40, -3, 70.4, 40, 1]),
    dict(
        type='Collect3D',
...
evaluation = dict(
    ...
    pipeline=[
        ...
        dict(type='Pad', size_divisor=32),
        dict(
            type='KittiSetOrigin', point_cloud_range=[0, -40, -3, 70.4, 40,
                                                      1]),
        dict(
            type='MultiScaleFlipAug3D',
            img_scale=(1333, 800),
...
optimizer = dict(
    type='AdamW', lr=0.000125, betas=(0.95, 0.99), weight_decay=0.01)
...
log_level = 'INFO'
load_from = 'pretrained_models/bev_painting_aug/epoch_40.pth'
resume_from = False
workflow = [('train', 1)]
work_dir = './work_dir/bevpainting_dataAug_add_bran_40puls'
gpu_ids = range(0, 1)
